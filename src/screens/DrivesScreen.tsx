      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <DriveCard
            ...
            onRunRoute={() => {
              // Go straight to the Record screen with the route data — no old pop-up
...
const DriveCard = memo(function DriveCard({
  ...
  const confirmDelete = () => {
    // keep the confirmation here; it's not the old run-route screen
    // (this is just a safety prompt for deletions)
    // biome-ignore lint:
